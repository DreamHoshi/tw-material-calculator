...
function render(){
  const result=calculateMaterials(recipes,active.name,active.amount,holdings), fragment=document.createDocumentFragment();let cost=0;
  ...
  names.forEach((name,index)=>{
    ...
    const subtotal=middle?0:needed*price;if(!Number.isSafeInteger(subtotal)||!Number.isSafeInteger(cost+subtotal))throw Error('費用が大きすぎます。');cost+=subtotal;
    ...
    const values=[middle?name:'',middle?'':name,fmt(required),'',fmt(needed),middle?'—':price?fmt(price):'未調査',middle?'—':price?fmt(subtotal):'未調査',middle?'不足分を制作':data.source||''];
  ...
  $('rows').replaceChildren(fragment);$('cost').textContent=fmt(cost);$('fee').textContent=fmt(result.fee);
}
